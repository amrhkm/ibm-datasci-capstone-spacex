...
# declare for heroku deployment. Needed for procfile.
server = app.server
...
def build_graph(site_dropdown):
    if site_dropdown == 'ALL':
        piechart = px.pie(data_frame=spacex_df, names='Launch Site', values='class',
                          title='Total Succesful Launches for All Sites')
        return piechart
    else:

        specific_df = spacex_df.loc[spacex_df['Launch Site'] == site_dropdown]
        piechart = px.pie(data_frame=specific_df, names='class', title='Total Succesful Launch for a Specific Site')
